...
#				animationList.append(["SwimDiveTurn","",-1,-1]);
				...
				animationList.append(["StrikeArms","",-1,-1]);
				animationList.append(["Strike2Arms","",-1,-1]);
				animationList.append(["Strike3Arms","",-1,-1]);
			...
			def updateSelection(self):
				...
				for bMeshObject in [bObject for bObject in Blender.Object.GetSelected() if (bObject.getType() == 'Mesh')]:
					...
					if self.selectedDict.has_key(name):
						continue
					self.scale = bObject.getSize("localspace");
					if bObject.getParent():
						self.scale = tuple([self.scale[0]*bObject.getParent().getSize("localspace")[0],self.scale[1]*bObject.getParent().getSize("localspace")[1],self.scale[2]*bObject.getParent().getSize("localspace")[2]])
					modelsize = self.scale
			...
			def export(self, exportPath, exportMaterial, materialScriptName, customMaterial, customMaterialTplPath, colouredAmbient, gameEngineMaterials, exportMesh, fixUpAxis, skeletonUseMeshName, convertXML, copyTextures):
				...
				if len(self.selectedList):
					...
					for name in self.selectedList:
						...
						# create MeshExporter
						meshExporter = MeshExporter(self.selectedDict[name], skeletonUseMeshName, self.scale)
						if self.morphAnimationProxyManagerDict.has_key(name):
