...
class Vertex:
	...
	def writePosition(self, fileObject, indentation=0):
		fileObject.write(indent(indentation) + "<position x=\"%.6f\" y=\"%.6f\" z=\"%.6f\"/>\n" \
			% ApplyScale(tuple(self.getPosition())))
		return
...
class MeshExporter:
	...
	"""
	def __init__(self, bObject, skeletonUseMeshName, scale):
		global modelsize
		"""
		"""
		# mesh
		...
		if (parent is not None):
			self.armatureExporter = ArmatureExporter(self.bObject, parent, skeletonUseMeshName, scale)
		# populated on export
